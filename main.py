    ...
    if text is not None:
        for word in words:
            if word.lower() in text.lower():
                ...
                await client.send_message(-1001989879370, send_text)
...
async def forward_messages_to_channel(client, message: types.Message):
    ...
    if message.from_user and message.chat.id != -1001989879370 and message.from_user.username\
        and check_db(message.text) :
